...
- 📨 **Встроенная почта** — приём и отправка писем, несколько ящиков, папки (входящие, отправленные, черновики, корзина, статистика доставки), статусы открытия/bounce/жалоб через Postbox, привязка писем к задачам через адреса вида task+ID@.
