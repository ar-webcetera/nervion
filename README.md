...
- 📨 **Встроенная почта** — приём и отправка писем, несколько изолированных ящиков, пользовательские папки и спам-фильтры, повторная отправка недоставленных писем, статусы открытия/bounce/жалоб через Postbox, привязка писем к задачам через адреса вида task+ID@.
