...
// Внешнее изменение таймлога (ресинк при возврате, WS-событие, остановка из десктопа/др. устройства):
// проп currentTimelog приходит из стора, а локальный timelog — снимок. Подхватываем актуальное
// состояние и пере-инициализируем тик (остановим, если сервер говорит, что таймер уже не идёт).
watch(
  () => props.timelog,
  (val) => {
    const sameState =
