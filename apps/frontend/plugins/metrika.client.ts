// Опциональная Яндекс.Метрика: грузится ТОЛЬКО если задан NUXT_PUBLIC_METRIKA_ID.
// По умолчанию выключена, в открытом коде никаких счётчиков нет. ID задаётся в env стенда.
interface YmFn {
  (...args: unknown[]): void;
  a?: unknown[];
  l?: number;
